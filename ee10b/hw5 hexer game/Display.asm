...
; Shared Variables:  LEDbuffer - (W) blank LEDs are written to each byte in buff
; Global Variables:  None.
;
; Input:             None.
; Output:            None.
...
StoreMasks:                      ; store the mask bytes to the buffer
    LDI     YL, LOW(LEDbuffer)      ; get the buffer address
    LDI     YH, HIGH(LEDbuffer)     
    ADIW    Y,  LED_DIG             ; offset buffer by start of the game LEDs 
    ST      Y+, R17                 ; store low byte and go to the high byte
    ST      Y, R16                  ; store high byte
